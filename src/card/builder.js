 ...
 */
// ── Patch 5: spinner phrases read from channels.feishu.spinnerPhrases per profile ──
// Each agent's openclaw.json owns its own phrase pool; the plugin no longer
// carries hardcoded arrays. See examples/spinner-phrases-*.example.json.
function randomSpinnerPhrase(cfg) {
    const phrases = cfg?.channels?.feishu?.spinnerPhrases;
    if (!Array.isArray(phrases) || phrases.length === 0) {
        return 'Processing...';
    }
    return phrases[Math.floor(Math.random() * phrases.length)];
}
function buildStreamingThinkingCard(showToolUse = true, cfg) {
    return buildStreamingPreAnswerCard({ showToolUse, cfg });
}
/**
...
function buildStreamingPreAnswerCard(params) {
    const { steps, elapsedMs, showToolUse = true, cfg } = params;
        ...
        columns: [
            {
                ...
                elements: [{
                    tag: 'markdown',
                    content: randomSpinnerPhrase(cfg),
                    text_size: 'notation',
                }],
            },
            {
    ...
    return {
        ...
        config: {
            ...
            summary: {
                content: randomSpinnerPhrase(cfg),
                i18n_content: { zh_cn: randomSpinnerPhrase(cfg), en_us: 'Processing...' },
            },
        },
        body: { elements },
    };
